...
const generateHtmlHeader = (pageTitle, currentPage) => {
    ...
    const structuredData = `<script type="application/ld+json">
    {
      ...
      "@type": "Person",
      "name": "${authorName}",
      "url": "https://DGE83.github.io/portfolio-3d-ia",
...
const generateHtmlFooter = () => `</main>
    <script src="js/lightgallery.min.js"></script>
    <script src="plugins/lg-thumbnail.min.js"></script>
    <script src="plugins/lg-zoom.min.js"></script>
    <script src="plugins/lg-video.min.js"></script>
...
const generateCreationCard = (creation, isGalleryItem) => {
    const filePath = creation.file_path; 

    let thumbnail = `<div class="stl-placeholder">Modèle 3D STL</div>`;
    if (creation.file_type === 'image') {
        thumbnail = `<img src="${filePath}" alt="${creation.title}" loading="lazy">`;
    } else if (creation.file_type === 'video') {
        thumbnail = `<video muted loop playsinline><source src="${filePath}" type="video/mp4"></video>`;
    }

    if (isGalleryItem && (creation.file_type === 'image' || creation.file_type === 'video')) {
        const dataAttributes = creation.file_type === 'video' 
            ? `data-html="#video-${creation.id}"` 
            : `data-src="${filePath}"`;

        const videoHtmlBlock = creation.file_type === 'video' ? `
            <div style="display:none;" id="video-${creation.id}">
                <video class="lg-video-object lg-html5" controls preload="none" playsinline muted autoplay>
                    <source src="${filePath}" type="video/mp4">
                    Your browser does not support HTML5 video.
                </video>
            </div>` : '';

        return `<div class="card gallery-item" ${dataAttributes} data-sub-html="<h4>${creation.title}</h4><p>${creation.description}</p>">
            <a href="${filePath}">
                <div class="card-thumbnail">${thumbnail}</div>
                <div class="card-content"><h3>${creation.title}</h3></div>
            </a>
            ${videoHtmlBlock}
        </div>`;
    }
    ...
    if (creation.file_type === 'stl') {
        return `<div class="card stl-card" data-file-path="${filePath}">
            <div class="card-thumbnail">${thumbnail}</div>
            <div class="card-content"><h3>${creation.title}</h3><p>${creation.description}</p></div>
        </div>`;
    }
...
const generateSitemap = () => {
    ...
    const siteUrl = "https://DGE83.github.io/portfolio-3d-ia"; 
    const pages = ['index.html', 'fusion360.html', 'blender.html', 'ia.html', 'contact.html'];
    const today = new Date().toISOString().split('T')[0];
    const sitemapContent = `<urlset xmlns="http://www.sitemaps.org/schemas/sitemap/0.9">${pages.map(page => `
    <url>
